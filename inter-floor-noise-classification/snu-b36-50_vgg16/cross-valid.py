...
pp  = {
    ...
    'freeze_layer':False,
    'pretrain_weights':'vgg16_weights.npz',
    'bn': False,
    ...
    'rec_name_summary':'result/vgg16-type-cv-summary.csv', # 'type' <-> 'position'
    'rec_name':'result/vgg16-type',                        # 'type' <-> 'position'
    'rec_name_cfm':'result/vgg16-type',                    # 'type' <-> 'position'
    'saver_name':'saver_vgg16-type',                       # 'type' <-> 'position'
    ...
    for fold in range(1, pp['num_fold']+1):
        ...
        for cnt in range(pp['random_search_cnt_max']):
            ...
            if pp['is_transfer_learn']:
                vgg = vgg16_adap.vgg16(imgs=imgs, img_mean=patch_mean, weights=pp['pretrain_weights'], sess=sess, bn=pp['bn'], bn_is_training=False, num_output=pp['n_category'])
            else:
                vgg = vgg16_adap.vgg16(imgs=imgs, img_mean=patch_mean, sess=sess, bn=pp['bn'], bn_is_training=True, num_output=pp['n_category'])

            # Logits, y_out, loss
            logits = vgg.fc4l
            y_out = tf.nn.softmax(logits)
            l2_loss_4w = penalty * tf.nn.l2_loss(vgg.fc4w)
            ...
            # Optimization
            if pp['freeze_layer']:
                train_op = tf.train.GradientDescentOptimizer(learning_rate=learning_rate).minimize(loss, var_list=[vgg.fc1w, vgg.fc1b,vgg.fc2w,vgg.fc2b,vgg.fc3w,vgg.fc3b,vgg.fc4w,vgg.fc4b])
            # Update all layers
            else:
            ...
            for epoch in range(pp['n_epoch']):
                ...
                if pp['saver'] and best_valid_flag:
                    vgg.save_weights(pp['saver_name']+'_k' + str(fold)+'.npz', sess)
