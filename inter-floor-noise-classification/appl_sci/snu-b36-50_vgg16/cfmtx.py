...
if __name__ == "__main__":
    ...
    metadata_path = '../dataset/metadata_position_fine3f.csv'  # where is meta-data?
    label_column_name = 'category'  # @metdata.csv, which index indicate category?

    # Get label dictionary from the metadata
    # - Dictionary
    traindata_dict = dict(metadata_path=metadata_path, label_column_name=label_column_name, split='1')
...
#4.
    ...
    draw2(file='result/vgg16-loc-fine3f-0-h0-f4-ep22.csv', normalize=True, xticks_ref=list(traindata_dict_label.values()), yticks_ref=list(testdata_dict_label.values()))
