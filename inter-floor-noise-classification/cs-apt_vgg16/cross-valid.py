...
pp  = {
    ...
    'name_rec_summary':'result/vgg16-floor-cv-summary.csv', # 'type' <-> 'position' (or 'upper_lower')
    'name_rec':'result/vgg16-floor',                        # 'type' <-> 'position' (or 'upper_lower')
    'name_rec_cfm':'result/vgg16-floor',                    # 'type' <-> 'position' (or 'upper_lower')
    'name_saver':'saver_vgg16-floor',                       # 'type' <-> 'position' (or 'upper_lower')

    'path_metadata':'metadata.csv',                         # 'type' <-> 'position' (or 'upper_lower')
    'path_data_train':'train_floor_3s_224_224',             # 'type' <-> 'position' (or 'upper_lower')
    'path_data_valid':'valid_floor_3s_224_224',             # 'type' <-> 'position' (or 'upper_lower')
...
for fold in range(1, pp['n_fold'] + 1):
    ...
    for cnt in range(pp['cnt_max_rand_search']):
        ...
        if pp['is_optimal_h']: lr_rand = 0.0015259075823746; reg_rand = 0.0070309188705686
